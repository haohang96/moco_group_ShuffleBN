...
class MoCo(nn.Module):
    ...
    """
    def __init__(self, s_encoder, t_encoder, dim=128, K=65536, m=0.999, T=0.07, mlp=False):
        """
        ...
        self.encoder_q = s_encoder(num_classes=dim)
        self.encoder_k = t_encoder(num_classes=dim)

        if mlp:  # hack: brute-force replacement
            dim_mlp = self.encoder_q.fc.weight.shape[1]
            self.encoder_q.fc = nn.Sequential(nn.Linear(512, 2048), nn.ReLU(), nn.Linear(2048,128))
